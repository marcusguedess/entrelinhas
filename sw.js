...
self.addEventListener('install', (event) => {
  event.waitUntil(Promise.all([self.skipWaiting(), caches.open(SHELL_CACHE).then((cache) => cache.addAll(SHELL))]));
});
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((names) =>
      Promise.all(
        names
          .filter((name) => name.startsWith('entrelinhas-') && ![SHELL_CACHE, MEDIA_CACHE].includes(name))
...
function isSafeAsset(value) {
  try {
    ...
    return url.origin === self.location.origin && /\.(?:mp3|m4a|ogg|wav|vtt|md)$/i.test(url.pathname);
  } catch {
    return false;
  }
}
